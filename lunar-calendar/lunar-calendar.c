...
static void lunar_calendar_class_init (LunarCalendarClass *klass)
{
     ...
     */
    calendar_signals[ACTIVATE] = g_signal_new ("activate",
...
static gchar* calendar_detail_cb (GtkCalendar *gcalendar, guint year,
                                  guint month, guint day, gpointer data)
{
    GError *error = NULL;
    gchar *value, *holiday;
    LunarCalendar *calendar;
    ...
    if (current_month == month) {
        ...
        value = g_strdup_printf ("<span size=\"x-small\" fgcolor=\"%s\">%s</span>",
                                 color, holiday);
        g_free (color);
    }
    g_free (holiday);
